...
// #include "c-secp256k1/include/govm.h"
...
*/
// #cgo LDFLAGS: -v ${SRCDIR}/c-secp256k1/.libs/libsecp256k1.a -lgmp
import "C"
...
// Serialize an ECDSA signature in compact format, returning the []byte
// and the recovery id. Return code is always 1.
func EcdsaRecoverableSignatureSerializeCompact1(ctx *Context, sig *EcdsaRecoverableSignature) (int, []byte, int, error) {
	output := make([]C.uchar, LenCompactSig+1)
	r := C.int(0)
	result := int(C.secp256k1_ecdsa_recoverable_signature_serialize_compact(ctx.ctx, &output[1], &r, sig.sig))
	output[0] = C.uchar(27 + r + 4)
	return result, goBytes(output, C.int(LenCompactSig+1)), int(r), nil
}

func GovmSha3(data []byte) []byte {
	output := make([]C.uchar, 256/8)
	C.govm_sha3(cBuf(data), C.ulong(len(data)), &output[0])
	return goBytes(output, C.int(256/8))
}

func GovmHashPower(data []byte) int {
	return int(C.govm_hash_power(cBuf(data), C.ulong(len(data))))
}

func GovmSolveOne(ctx *Context, block []byte, key []byte) ([]byte, []byte) {
	output_result := make([]C.uchar, 1+SignLen+len(block))
	output_hash := make([]C.uchar, 32)
	...
	*/
	...
	return goBytes(output_result, C.int(len(output_result))), goBytes(output_hash, C.int(len(output_hash)))
}

func GovmSolveMany(ctx *Context, block []byte, key []byte, count uint64) ([]byte, []byte, uint64) {
	output_result := make([]C.uchar, 1+SignLen+len(block))
	output_hash := make([]C.uchar, 32)
	...
	*/
	...
	nonce := uint64(C.govm_block_best(
		ctx.ctx,
		C.ulong(count),
		cBuf(block),
		C.ulong(len(block)),
	...
	return goBytes(output_result, C.int(len(output_result))), goBytes(output_hash, C.int(len(output_hash))), nonce
}
